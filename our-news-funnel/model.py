    ...
    def predict(self, articles, method="beam"):
        if method == "greedy":
            ...
            for i in range(self.config.summary_length):
                ...
                reselected_padded_predictions = tf.gather_nd(padded_predictions, best_beams)
                print reselected_padded_predictions.get_shape()
                assert reselected_padded_predictions.get_shape() == (self.config.batch_size, self.beam_size, self.config.context_size + i)
